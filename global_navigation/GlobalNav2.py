...
def get_everything_from_vision(video_stream, QR_detector, transformation_matrix, recalculate_global, obstacle_vertices,
                               goal_center,
                               window_name, return_position):
    image_detected, image = video_stream.read()
    if image_detected:
        image_initial = image.copy()
        height, width, channels = image.shape
        ...
        robot_angle, robot_center, qr_vertices = get_robot_pos_angle(new_perspective_image, QR_detector,
                                                                     transformation_matrix)
        if return_position:
            return np.concatenate(robot_center, robot_angle)
        ...
        if robot_center is not None:
            ...
            new_perspective_image = cv2.polylines(new_perspective_image, [qr_vertices.astype(int)], isClosed=True,
